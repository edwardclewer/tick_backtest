...
def compute_performance_metrics(df: pd.DataFrame) -> dict[str, object]:
    ...
    gross_profit_pips = _as_float(metrics["gross_profit_pips"])
    gross_loss_pips = _as_float(metrics["gross_loss_pips"])
    metrics["profit_factor"] = (
        gross_profit_pips / abs(gross_loss_pips)
        if gross_loss_pips < 0
        else math.inf if gross_profit_pips > 0
        else float("nan")
    )
...
def format_metrics(metrics: dict[str, object]) -> list[tuple[str, str]]:
    ...
    return [
        ...
        ("Win rate", f"{_as_float(metrics['win_rate'])*100:.2f}%" if not math.isnan(_as_float(metrics['win_rate'])) else "NaN"),
...
def plot_equity_curve(df: pd.DataFrame, plot_path: Path) -> Path | None:
    if plt is None:
        return None
